...
(function() {
    ...
    // DOM elements
    overlay, center, title, response_div, sending_div, container, style, status,
    ...
    ContactFormBox = {
        ...
        open: function(_media, _options) {
            options = {
                ...
                correctErrors: 'Please correct error(s)'
            };

            closeLink.set('html', options.closeButtonText);
        ...
        changeMedia: function(_media, _options) {
            ...
            media.set('html', '');
            title.set('html', _options.sysMsg?'<span style="color:#f00;">' + _options.sysMsg + '</span>':'');
        ...
        sendMessage: function(_form, _options){
            ...
            if (document.formvalidator.isValid(form)) {
                if(status == statuses.closed){
                    sending_div.set('html', '<div class="cfp_msg_inner"><div class="cfp_msg_inner_top"><span class="cfp_msg_txt">'+_options.sendingMessage+'</span></div><div class="cfp_msg_inner_bot cfp_msg_sending"></div></div>');
                    ContactFormBox.open(sending_div, _options);
                }else{
                    sending_div.set('html', '<div class="cfp_msg_inner"><div class="cfp_msg_inner_top"><span class="cfp_msg_txt">'+options.sendingMessage+'</span></div><div class="cfp_msg_inner_bot cfp_msg_sending"></div></div>');
                ...
                new Request.JSON({
                    ...
                    onSuccess: function(response){
                        if(_options.display == 'form' || response.status == 1){
                            ...
                            // Erreur, go back
                            ContactFormBox.changeMedia(mainForm, {sysMsg: response.message});
                        }
                ...
                if(status == statuses.closed){
                    response_div.set('html', '<div class="cfp_msg_inner"><div class="cfp_msg_inner_top"><div class="cfp_msg_res failure"><span class="cfp_msg__res_txt">' + _options.correctErrors + '</span></div></div></div>');
                    ContactFormBox.open(response_div, {});
                }
                else{
                    title.set('html', '<span style="color:#f00;">' + _options.correctErrors + '</span>');
                }
            }
